// ✅ Ultra-optimized version of quiz.js — fonctionnelle avec bouton de validation intégré
...
const ui = {
  ...
  render(q, selected, onChange, onSubmit) {
...
async function submitAnswer() {
  if (!state.current || state.choices.size === 0) return ui.showToast("Veuillez sélectionner une réponse.");
  try {
    ...
    if (data.success) {
      ...
      ui.showToast("Réponse enregistrée !");
    } else {
      throw new Error("Réponse refusée");
    }
  } catch (e) {
...
async function fetchQuestion() {
  ...
  try {
    ...
    if (data.success && data.question?.id !== state.lastId) {
      ...
      ui.render(q, state.choices, onChoiceChange, submitAnswer);
      ui.hideLoading();
    }
  } catch (e) {
